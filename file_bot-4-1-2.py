...
def get_chrome_driver(proxy: dict = None) -> tuple:
    ...
    found_drv = None
    for drv_path in ["/usr/bin/chromedriver", "/usr/bin/chromium-driver", "/usr/local/bin/chromedriver"]:
        if os.path.exists(drv_path):
            found_drv = drv_path
            logger.info(f"Found chromedriver at: {drv_path}")
            try:
    ...
    if not found_drv:
        logger.error("No chromedriver found in any known path!")
