...
def get_session(user_id: int) -> dict:
    if user_id not in user_sessions:
        user_sessions[user_id] = {
            "proxy_type": None,      # "9proxy" | "abc" | None
            "proxy_user": None,
            "proxy_pass": None,
            "proxy_host": None,
            "proxy_port": None,
            "threads": 3,
            "password_mode": "random",  # "random" | "manual"
...
async def voltx_get_ranges() -> list:
    """Get active Facebook ranges from VOLTX."""
    try:
...
async def voltx_get_number(range_val: str) -> Optional[dict]:
    """Get a number from VOLTX for given range."""
    try:
        async with httpx.AsyncClient(timeout=15) as client:
...
async def voltx_check_otp(number: str, wait: int = 90) -> Optional[str]:
    """Poll VOLTX console for OTP for given number."""
...
def parse_proxy(proxy_str: str, proxy_type: str) -> dict:
    """Parse username:password proxy input."""
...
def _proxy_extension(proxy: dict) -> Optional[str]:
    """Create Chrome proxy extension for authenticated SOCKS5."""
    try:
...
async def create_fb_account(number: str, password: str, proxy: dict = None) -> dict:
    """Create Facebook account using Selenium. Returns result dict."""
...
async def submit_otp_to_fb(driver, otp: str) -> bool:
    """Submit OTP code to Facebook verification page."""
    try:
        ...
        click_el(driver, By.XPATH, "//button[@type='submit']")
        await asyncio.sleep(3)
        return True
    except Exception as e:
...
async def worker(user_id: int, bot, thread_id: int):
    """Single thread worker for account creation."""
    ...
    while sess["running"]:
        try:
            ...
            password = sess["manual_password"] if sess["password_mode"] == "manual" and sess["manual_password"] else gen_password()

            # 2. Create FB account (in thread executor)
            loop = asyncio.get_event_loop()
            fb_result = await loop.run_in_executor(
                None, lambda: asyncio.run(create_fb_account(number, password, proxy))
            )
